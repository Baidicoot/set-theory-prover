...
type ProverState = (Keywords, NotationRHS, Grammar, Env, Maybe (Term, Proof, [ElabCtx]))

serializeProverState :: (Keywords,NotationRHS,Grammar,Env) -> Prover B.ByteString
serializeProverState (kw,nr,g,e) = pure (encode (kw,nr,g,e))

deserializeProverState :: B.ByteString -> Prover (Keywords, NotationRHS, Grammar, Env)
deserializeProverState b = case decodeOrFail b of
    Left (_,_,e) -> throwError (Serializer e)
    Right (b,_,(kw,nr,g,e)) | B.null b -> pure (kw,nr,g,e)
...
compileGrammar (_,_,grammar, _, _) = do
    x <- gets snd
    case x of
        Just g -> pure g
        Nothing -> do
...
initialState = (S.empty, nrINIT, grINIT, (Sorts mempty, ConstObjs mempty, DefObjs mempty, Axioms mempty), Nothing)

insertNonterminal :: Name -> Name -> ProverState -> ProverState
insertNonterminal n rhs (kw, nr, grammar, env, goal) =
    (kw, M.insert n rhs nr, M.insert n [] grammar, env, goal)

insertAxiom :: Name -> Term -> ProverState -> ProverState
insertAxiom n m (kw, nr, grammar, (s,o,d,Axioms a), goal) =
    (kw, nr, grammar, (s,o,d,Axioms (M.insert n m a)), goal)

insertSort :: Name -> ProverState -> ProverState
insertSort n (kw, nr, grammar, (Sorts s,o,d,a), goal) =
    (kw, nr, grammar, (Sorts (S.insert n s),o,d,a), goal)

insertConst :: Name -> Polytype -> ProverState -> ProverState
insertConst n t (kw, nr, grammar, (s,ConstObjs o,d,a), goal) =
    (kw, nr, grammar, (s,ConstObjs (M.insert n t o),d,a), goal)

insertDef :: Name -> Polytype -> Term -> ProverState -> ProverState
insertDef n m t (kw, nr, grammar, (s,o,DefObjs d,a), goal) =
    (kw, nr, grammar, (s,o,DefObjs (M.insert n (m,t) d),a), goal)

insertKeyword :: Name -> ProverState -> ProverState
insertKeyword k (kw, nr, grammar, env, goal) = (S.insert k kw, nr, grammar, env, goal)
...
checkProof (_, _, _, env, _) prop prf = do
    names <- getNames
...
checkProp (_, _, _, env, _) sort prop = do
    names <- getNames
...
refine (_, _, _, _, Nothing) _ = throwError NotInProofMode
refine (kw, nr, grammar, env, Just (prop, prf, _:ctxs)) (p, ctxs') = case fillHole prf p of
    Just prf' ->
        let state' = (kw, nr, grammar, env, Just (prop, prf', ctxs' ++ ctxs))
        in do
...
addNotation (kw, nr, grammar, env, p) n ns s = do
    case makeProdRule n ns s of
        Left err -> throwError (Parser err)
        Right prod -> uncompileGrammar >> pure (kw, nr, M.adjust (++[prod]) n grammar, env, p)

beginProof :: ProverState -> Term -> Prover ProverState
beginProof (kw, nr, grammar, env, Nothing) t = pure (kw, nr, grammar, env, Just (t, Hole, [mempty]))
beginProof _ _ = throwError InProofMode

endProof :: ProverState -> T.Text -> Prover ProverState
endProof (_, _, _, _, Nothing) t = throwError NotInProofMode
endProof (_, _, _, _, Just (_, _, _:_)) _ = throwError OpenGoals
endProof s@(kw, nr, gr, env, Just (prop, _, [])) t =
    pure (insertAxiom t prop (kw, nr, gr, env, Nothing))

goalCtx :: ProverState -> Prover ElabCtx
goalCtx (_, _, _, env, Just (_, _, ctx:_)) = pure (ctx `M.union` envToElabCtx env)
goalCtx (_, _, _, _, Just _) = throwError NoOpenGoals
goalCtx _ = throwError NotInProofMode

envCtx :: ProverState -> ElabCtx
envCtx (_, _, _, env, _) = envToElabCtx env

parseProof :: ProverState -> Grammar -> ElabCtx -> T.Text -> Prover (Proof, [ElabCtx], ProverState)
parseProof s@(kw, nr, grammar, env, p) g ctx t = do
    case parse kw g ntPROOF t of
        Left err -> throwError (Parser err)
        Right s -> do
            names <- getNames
            case runElaborator names ctx (elabProof s) of
                ((Right o, ctx'), names') -> putNames names' >> pure (o, ctx', (kw, nr, grammar, env, p))
                ((Left err, _), _) -> throwError (Parser err)

parseMonotype :: ProverState -> Grammar -> ElabCtx -> T.Text -> Prover (Monotype, ProverState)
parseMonotype (kw, nr, grammar, env, p) g ctx t =
    case parse kw g ntSORT t of
        Left err -> throwError (Parser err)
        Right s -> do
            names <- getNames
            case runElaborator names ctx (elabMonotype s) of
                ((Right o, _), names') -> putNames names' >> pure (o, (kw, nr, grammar, env, p))
                ((Left err, _), _) -> throwError (Parser err)

parseProp :: ProverState -> Grammar -> ElabCtx -> T.Text -> Prover (Term, ProverState)
parseProp (kw, nr, grammar, env, p) g ctx t =
...
parseSort (kw, nr, grammar, env, p) g ctx t =
    case parse kw g ntSORT t of
        Left err -> throwError (Parser err)
        Right s -> do
            names <- getNames
...
parsePolytype (kw, nr, grammar, env, p) g ctx t =
    case parse kw g ntSORT t of
        Left err -> throwError (Parser err)
        Right s -> do
            names <- getNames
...
parseNotationBinding (kw, nr, grammar, env, p) g t =
...
parseNotationProduct (kw, nr, grammar, env, p) n t = do
    ns <- getNames
    g <- case elimLeftRec ns (addPlaceholders (M.keysSet nr) grammar) of
            (Right g,ns) -> do
                putNames ns
                pure g
            (Left err,ns) -> throwError (Parser err)
    case M.lookup n nr of
        Just rhs -> do
            case parse kw g rhs t of
                Left err -> throwError (Parser err)
                Right s -> pure s
        Nothing -> throwError (Parser (NotNonterminal n))
...
newNonterminalExt :: ProverState -> (T.Text, T.Text) -> Prover ProverState
newNonterminalExt s (n, rhs) = pure (insertNonterminal n rhs s)
...
printGrammarExt s@(_, _, g, _, _) = liftIO (print (fmap (fmap snd) g)) >> pure s

doneExt :: ProverState -> Prover ProverState
doneExt (_, _, _, _, Nothing) = throwError NotInProofMode
doneExt (_, _, _, _, Just (_, _, _:_)) = throwError OpenGoals
doneExt s@(_, _, _, _, Just (prop, _, [])) =
    pure s

dieExt :: ProverState -> Prover ProverState
dieExt _ = throwError Terminated

updatedState :: Keywords -> NotationRHS -> Grammar -> Env -> Prover ProverState
updatedState kw nr g e = do
    uncompileGrammar
    pure (kw, nr, g, e,Nothing)

includeStateExt :: ProverState -> B.ByteString -> Prover ProverState
includeStateExt (kw,nr,g,e,Nothing) b = do
    (kw', nr', g',e') <- deserializeProverState b
    updatedState (kw `S.union` kw') (nr `M.union` nr') (g `M.union` g') (e <> e')
includeStateExt _ _ = throwError InProofMode

loadStateExt :: ProverState -> B.ByteString -> Prover ProverState
loadStateExt (_,_,_,_,Nothing) b = do
    (kw,nr,g,e) <- deserializeProverState b
    updatedState kw nr g e
loadStateExt _ _ = throwError InProofMode

dumpStateExt :: ProverState -> Prover (ProverState,B.ByteString)
dumpStateExt (kw,nr,g,e,Nothing) = do
    b <- serializeProverState (kw,nr,g,e)
    pure ((kw,nr,g,e,Nothing),b)
